...
def generate_historical_data(days=30, records_per_day=48, direction='past'):
    """
    ...
    Args:
        ...
        direction: 'past' 生成过去数据；'future' 生成未来数据
    """
    print(f"开始生成 {days} 天的人数检测历史数据（仅id<=19的区域，方向：{'未来' if direction=='future' else '过去'}）...")
    ...
    minutes_per_record = int(1440 / records_per_day) if records_per_day > 0 else 30
    ...
    for area in areas:
        ...
        for day in range(days):
            for record in range(records_per_day):
                # 计算时间戳（UTC+8时区）
                # 过去：now - [(days - day - 1) 天 + record*间隔]
                # 未来：now + [day 天 + record*间隔]
                timestamp = now + ( -1 if direction == 'past' else 1 ) * timedelta(
                    days=(days - day - 1) if direction == 'past' else day,
                    minutes=record * minutes_per_record
                )
...
def generate_temperature_humidity_data(days=30, records_per_day=48, direction='past'):
    """
    ...
    Args:
        ...
        direction: 'past' 生成过去数据；'future' 生成未来数据
    """
    print(f"开始生成 {days} 天的温湿度数据（仅id<=19的区域，方向：{'未来' if direction=='future' else '过去'}）...")
...
def generate_co2_data(days=30, records_per_day=48, direction='past'):
    """
    生成CO2数据记录（绑定终端）
    ...
    Args:
        ...
        direction: 'past' 生成过去数据；'future' 生成未来数据
    """
    print(f"开始生成 {days} 天的CO2数据（方向：{'未来' if direction=='future' else '过去'}）...")
...
def main():
    ...
    # 获取生成参数（改为分别指定过去/未来天数）
    try:
        past_days = int(input("请输入要生成多少天的过去数据 (默认30天): ") or "30")
        future_days = int(input("请输入要生成多少天的未来数据 (默认0天): ") or "0")
        records_per_day = int(input("请输入每天生成多少条记录 (默认48条，即每30分钟一条): ") or "48")
    except ValueError:
        print("输入无效，使用默认值：过去30天、未来0天，每天48条记录")
        past_days = 30
        future_days = 0
        records_per_day = 48

    # 移除时间方向选择，改为分别指定过去/未来天数
    # ...删除原先的 time_dir 相关代码...
    ...
    print(f"\n将生成 过去{past_days}天 + 未来{future_days}天，每天 {records_per_day} 条记录（UTC+8时区）")
    ...
    if choice in ['1', '2', '5']:
        total_historical = area_count * (past_days + future_days) * records_per_day
    if choice in ['1', '3', '5']:
        total_temp_humid = area_count * (past_days + future_days) * records_per_day
    if choice in ['1', '4']:
        total_co2 = terminal_count * (past_days + future_days) * records_per_day
    ...
    try:
        ...
        if choice in ['1', '2', '5'] and area_count > 0:
            if past_days > 0:
                generate_historical_data(past_days, records_per_day, direction='past')
            if future_days > 0:
                generate_historical_data(future_days, records_per_day, direction='future')
        
        if choice in ['1', '3', '5'] and area_count > 0:
            if past_days > 0:
                generate_temperature_humidity_data(past_days, records_per_day, direction='past')
            if future_days > 0:
                generate_temperature_humidity_data(future_days, records_per_day, direction='future')
        
        if choice in ['1', '4'] and terminal_count > 0:
            if past_days > 0:
                generate_co2_data(past_days, records_per_day, direction='past')
            if future_days > 0:
                generate_co2_data(future_days, records_per_day, direction='future')
